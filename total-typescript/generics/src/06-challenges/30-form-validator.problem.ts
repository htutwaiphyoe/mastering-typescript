...
const makeFormValidatorFactory =
  <T extends string>(validators: Record<T, (value: string) => string | void>) =>
  <U extends string>(config: Record<U, T[]>) => {
    return (values: Record<U, string>) => {
      const errors = {} as Record<U, string | undefined>;
